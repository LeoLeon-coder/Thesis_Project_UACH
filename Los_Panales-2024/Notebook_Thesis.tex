...
\begin{itemize}[label=\ding{43}]
	...
	\item \texttt{queen status}: una etiqueta combinada derivada de las dos anteriores, con los siguientes valores: 0 (original/con reina funcional), 1 (no presente), 2 (presente y rechazada), 3 (presente y recién aceptada). Esta es la etiqueta principal utilizada para la clasificación en este trabajo.
	\item \texttt{time}: la hora del día, escalada entre 0 y 1 para representar un ciclo de 24 horas.
\end{itemize}
...
\section{Real-time decision logic}

The model predicts per 2-second segment and since real life needs stability temporal smoothing is applied as follows:
\begin{enumerate}
	\item Every 1 second:
	\begin{enumerate}[label=\roman*.]
		\item take last 2 seconds of audio,
		\item compute features,
		\item predict class probabilities $p_t$.
	\end{enumerate}
	\item Keep a rolling window of the last N predictions (e.g., 60 seconds $\to N\approx 60$):
	\begin{itemize}[label=\ding{43}]
		\item average probabilities:
		\begin{equation}\label{av-prob}
			\bar{p} = \frac{1}{N}\sum_{t=1}^{N} p_t \; .
		\end{equation}
		\item Output class $= argmax(\bar{p})$.
		\item Confidence $= max(\bar{p})$.
	\end{itemize}
\end{enumerate}
%-------------------------------------------------------------------------------------
% BIBLIOGRAFÍA.
